...
public sealed class SessionStore
{
    ...
    public async Task AppendAsync(string sessionId, TelemetrySnapshot s)
    {
        ...
        try
        {
            ...
            if (!exists)
                await sw.WriteLineAsync("timestamp,driver,vehicle,track,lap,speed_kph,throttle,brake,steering,gear,fuel_l,in_garage,in_pit_lane");
            await sw.WriteLineAsync(string.Join(',',
                s.Timestamp.ToString("O", CultureInfo.InvariantCulture), Csv(s.Driver), Csv(s.Vehicle), Csv(s.Track), s.Lap,
